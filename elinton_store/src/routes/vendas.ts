...
const itemVendaSchema = z.object({
  ...
  qtd: z.number().int().positive({ message: "A quantidade deve ser positiva." })
})

const vendaSchema = z.object({
  clienteId: z.number().int().positive(),
  itens: z.array(itemVendaSchema).min(1, { message: "A venda deve conter pelo menos um item." })
})

// GET /vendas - lista todas as vendas com cliente + itens + produtos aninhados
...
// POST /vendas - cria uma nova venda em transação (rota protegida)
...
// DELETE /vendas/:id - devolução em transação reversa (rota protegida)
